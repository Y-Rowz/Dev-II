...
def generer_graphique(df: pd.DataFrame, chemin: str) -> None:
    ...
    if not chemin or chemin.strip() == '':
        raise ValueError("Un nom de fichier est requis pour sauvegarder le graphique. Veuillez fournir un nom de fichier valide.")
    
    # Vérifier que le chemin a une extension valide
    if not chemin.lower().endswith(('.png', '.jpg', '.jpeg')):
        raise ValueError(f"Le chemin de l'image '{chemin}' doit avoir une extension valide (.png, .jpg, .jpeg)")
...
def generer_rapport_pdf(df: pd.DataFrame, chemin_pdf: str, chemin_graphique: str) -> None:
    ...
    if not os.path.exists(chemin_graphique):
        raise FileNotFoundError(f"Le fichier graphique spécifié '{chemin_graphique}' n'existe pas")
