import os
from src.rapport import generer_graphique, generer_rapport_pdf, ajouter_extension_si_absente
    ...
    while True:
        ...
        if choix == "1":
            ...
            if df is not None:
                ...
                prix_min_input = input(
                    "Prix minimum souhaité (laisser vide si non applicable) : "
                )
                prix_min = float(prix_min_input) if prix_min_input else None

                prix_max_input = input(
                    "Prix maximum souhaité (laisser vide si non applicable) : "
                )
            ...
            data_to_use = (
                df_filtré if df_filtré is not None and not df_filtré.empty else df
            )

            if data_to_use is not None:
                chemin_graphique = input(
                    "Nom du fichier pour sauvegarder le graphique (sans extension) : "
                )
                chemin_graphique = ajouter_extension_si_absente(
                    chemin_graphique, ".png"
                )
                try:
                ...
                chemin_pdf = input(
                    "Nom du fichier pour sauvegarder le rapport PDF (sans extension) : "
                )
                chemin_pdf = ajouter_extension_si_absente(chemin_pdf, ".pdf")
                try:
                    generer_rapport_pdf(data_to_use, chemin_pdf, chemin_graphique)
                    print(
                        f"Le rapport PDF a été créé avec succès : "
                        f"{os.path.abspath(chemin_pdf)}"
                    )
